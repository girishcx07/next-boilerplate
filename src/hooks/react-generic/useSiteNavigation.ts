...
import type { NavigationEntry, NavigationKeys, RichTranslationValues } from '@/types';
...
const replaceLabelWithContext = (label: string, context?: RichTranslationValues) =>
  Object.entries(context || {}).reduce(
    (finalLabel, [find, replace]) =>
      finalLabel.replace(`{${find}}`, typeof replace === 'string' ? replace : ''),
    label
  );
